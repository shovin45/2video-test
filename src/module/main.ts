...
export function main() {
  ...
  const controller = new MediaController(
    player.querySelectorAll('.preview > video'),
  )
  ...
  controller.onloadeddata = () => {
    ...
    for (const segment of Array.from(clip.querySelectorAll('span'))) {
      ...
      if (startTimeStr && endTimeStr) {
        ...
        segment.textContent = `${convertTimeToHumanReadable(
          startTime,
        )} - ${convertTimeToHumanReadable(endTime)}`
      }
    }
    ...
    controller.primaryMedia.addEventListener('timeupdate', () => {
      if (
        (clip.querySelector('[data-play-segments]') as HTMLInputElement).checked
      ) {
        let inRange = false
        let nearbyEndTime = 0
        let nextIndex = 0
        ...
        for (let i = 0; i < clipSegments.length; i++) {
          ...
          if (
            ...
            endTime >= controller.currentTime
          ) {
            inRange = true
            break
          } else if (
            endTime < controller.currentTime &&
            endTime > nearbyEndTime
          ) {
      ...
      currentTime.setAttribute('data-current-time', '' + controller.currentTime)
      currentTime.textContent = convertTimeToHumanReadable(
